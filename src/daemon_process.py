...
from AI_news_client import AINewsClient  # 导入 AI 新闻客户端
...
def ai_news_job(ai_news_client, report_generator, notifier):
    LOG.info("[开始执行定时任务]AI 新闻总结")
    news_file_path = ai_news_client.get_latest_news_file()
    if news_file_path:
        report, _ = report_generator.generate_ai_news_report(news_file_path)
        notifier.notify_ai_news_report(report)
    else:
        LOG.warning("没有找到最新的 AI 新闻文件")
    LOG.info("[定时任务执行完毕]AI 新闻总结")
...
def main():
    ...
    LOG.info(f"report_generator: {config.report_types}")

    subscription_manager = SubscriptionManager(config.subscriptions_file)  # 创建订阅管理器实例
    ai_news_client = AINewsClient()  # 创建 AI 新闻客户端实例
    ...
    ai_news_job(ai_news_client, report_generator, notifier)
    ...
    # 安排 AI 新闻任务每天执行一次，比如在每天下午 2 点
    schedule.every().day.at("14:00").do(ai_news_job, ai_news_client, report_generator, notifier)
