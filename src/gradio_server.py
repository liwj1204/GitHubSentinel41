...
from AI_news_client import AINewsClient
...
def generate_ai_news_report(model_type, model_name):
    config.llm_model_type = model_type
    ...
    if news_file_path:
        report, report_file_path = report_generator.generate_ai_news_report(news_file_path)
        return report, report_file_path
    else:
        return "未找到最新的 AI 新闻文件", None
...
def update_model_list(model_type):
    if model_type == "openai":
        ...
        return gr.Dropdown(choices=["llama3.1:8b", "gemma2:latest", "qwen2:latest"], label="选择模型")
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("AI News"):
        ...
        # 创建按钮来生成报告
        button = gr.Button("生成 AI News 总结")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_ai_news_report, inputs=[model_type, model_name], outputs=[markdown_output, file_output])
